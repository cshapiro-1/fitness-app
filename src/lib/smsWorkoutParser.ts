...
/**
 * High-Capacity Bulk Android Parser for Multi-Year Backups (.xml, .json, WhatsApp / Samsung .txt exports)
 */
export function parseBulkAndroidFile(
  rawContent: string,
  knownClients?: { id: string; name: string }[]
): { sessions: ParsedWorkoutSession[]; totalMessagesScanned: number } {
  if (!rawContent || !rawContent.trim()) {
    return { sessions: [], totalMessagesScanned: 0 };
  }

  // 1. Android "SMS Backup & Restore" XML format
  if (rawContent.includes("<sms ") || rawContent.includes("<smses")) {
    return parseSMSBackupXML(rawContent, knownClients);
  }

  // 2. WhatsApp / Samsung Messages multi-year exported text format
  const chatLineRegex = /^(?:\[\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4},?\s+\d{1,2}:\d{2}(?::\d{2})?\s*(?:[ap]m)?\]|\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4},?\s+\d{1,2}:\d{2}\s*(?:[ap]m)?\s*[\-\–])\s*([^:]+):\s*(.+)$/i;
  const lines = rawContent.split(/\r?\n/).filter(Boolean);
  const isChatExport = lines.slice(0, 30).some((l) => chatLineRegex.test(l));

  if (isChatExport) {
    return parseChatExport(rawContent, knownClients);
  }

  // 3. Standard multi-session text fallback
  const sessions = parseSMSWorkoutText(rawContent, undefined, knownClients);
  return { sessions, totalMessagesScanned: lines.length };
}

/**
 * Parses XML generated by Android "SMS Backup & Restore" app
 */
export function parseSMSBackupXML(
  xmlContent: string,
  knownClients?: { id: string; name: string }[]
): { sessions: ParsedWorkoutSession[]; totalMessagesScanned: number } {
  ...
  const workoutBuckets: Map<string, { date: string; clientName: string; clientId?: string; textBlocks: string[] }> = new Map();

  while ((match = smsTagRegex.exec(xmlContent)) !== null) {
    totalMessagesScanned++;
    const attrString = match[1];

    const bodyMatch = attrString.match(/body="([^"]*)"/i);
    const dateMatch = attrString.match(/date="(\d+)"/i);
    const contactMatch = attrString.match(/contact_name="([^"]*)"/i);
    const addressMatch = attrString.match(/address="([^"]*)"/i);

    if (!bodyMatch || !dateMatch) continue;
    ...
    // Quick filter: only process if message looks like a workout log
    const hasWorkoutIndicator = /\b(\d+\s*x\s*\d+|bench|squat|press|deadlift|curl|pushdown|lbs|reps|sets|pull\s*ups?|rows?|lunges?)\b/i.test(rawBody);
    if (!hasWorkoutIndicator) continue;

    const epochMs = parseInt(dateMatch[1], 10);
    const dateObj = new Date(epochMs);
    const dateStr = dateObj.toISOString().split("T")[0];

    const contactName = (contactMatch && contactMatch[1] && contactMatch[1] !== "(Unknown)")
      ? contactMatch[1].trim()
      : (addressMatch ? addressMatch[1].trim() : "Client");

    // Match with known clients
    let matchedClientId: string | undefined;
    let finalClientName = contactName;
    ...
    // Cluster by Client + Date (e.g. "Collin Shapiro__2023-08-10")
    const bucketKey = `${finalClientName}__${dateStr}`;
    if (!workoutBuckets.has(bucketKey)) {
      workoutBuckets.set(bucketKey, {
        date: dateStr,
    ...
    workoutBuckets.get(bucketKey)!.textBlocks.push(rawBody);
  }

  const sessions: ParsedWorkoutSession[] = [];

  for (const bucket of workoutBuckets.values()) {
    const combinedText = bucket.textBlocks.join("\n");
    const parsed = parseSMSWorkoutText(combinedText, bucket.date, knownClients);
    ...
    for (const s of parsed) {
      if (s.exercises.length > 0) {
        sessions.push({
          ...s,
          date: bucket.date,
          clientId: bucket.clientId || s.clientId,
          clientName: bucket.clientName || s.clientName,
        });
      }
    }
  }

  return { sessions, totalMessagesScanned };
}

/**
 * Parses exported chat transcripts (WhatsApp, Samsung Messages, Google Messages)
 */
export function parseChatExport(
  ...
  for (const line of lines) {
    ...
    const hasWorkoutIndicator = /\b(\d+\s*x\s*\d+|bench|squat|press|deadlift|curl|pushdown|lbs|reps|sets|pull\s*ups?|rows?|lunges?)\b/i.test(message);
    if (!hasWorkoutIndicator) continue;

    const normalizedDate = normalizeDateString(rawDate);

    let matchedClientId: string | undefined;
    let finalClientName = sender;

    if (knownClients && knownClients.length > 0) {
      const found = knownClients.find(
        (c) =>
          c.name.toLowerCase() === sender.toLowerCase() ||
          sender.toLowerCase().includes(c.name.toLowerCase()) ||
          c.name.toLowerCase().includes(sender.toLowerCase())
      );
      if (found) {
        matchedClientId = found.id;
        finalClientName = found.name;
    ...
    const bucketKey = `${finalClientName}__${normalizedDate}`;
    if (!workoutBuckets.has(bucketKey)) {
      workoutBuckets.set(bucketKey, {
        date: normalizedDate,
        clientName: finalClientName,
    ...
    workoutBuckets.get(bucketKey)!.textBlocks.push(message);
  }
